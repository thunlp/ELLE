...
def synchronize_training_time(training_time):
    """
    Helper function to synchronize between multiple processes when
    using distributed training
    """
...
def train(args, trainer, task, epoch_itr, cur_domain=None, recover=False):
    ...
    for i, samples in enumerate(progress, start=epoch_itr.iterations_in_epoch):
        ...
        '''
        while training_time == 0:
            time.sleep(1)
        if torch.distributed.get_rank() != 1:
            training_time = 0
        torch.distributed.broadcast(training_time_tensor, 1)
        training_time = training_time_tensor.item()
        while training_time == 0:
            time.sleep(1)
        '''
        ...
        if (
            ...
            valid_loss_list = {'training time': training_time, 'loss':[], 'on time': False}
        ...
        if (
            ...
            if name not in args.valid_loss_list.keys():
                args.valid_loss_list[name] = []
            valid_loss_list = {'training time': training_time, 'loss':[], 'on time': True}
...
def validate(args, trainer, task, epoch_itr, subsets, cur_domain=None):
    ...
    return valid_losses, stats['ppl'], stats['nll_loss'].avg
