...
WARMUP_RATIO=0.08    # Warmup the learning rate over this many updates
PEAK_LR=0.0005        # Peak learning rate, adjust as needed
TOKENS_PER_SAMPLE=512   # Max sequence length
MAX_POSITIONS=512       # Num. positional embeddings (usually same as above)
MAX_SENTENCES=16        # Number of sequences per batch (batch size)
UPDATE_FREQ=16          # Increase the batch size 16x
arch=roberta_6layer_384hidden_6head_1536ffn
logdir=log/roberta_base_ELLE
save_dir=checkpoints/roberta_base_ELLE
...
python ../../fairseq_cli/train.py --fp16 $DATA_DIR \
		...
		--lr-scheduler polynomial_decay --lr $PEAK_LR --warmup-ratio $WARMUP_RATIO --total-num-update $TOTAL_UPDATES --recover-update 5000\
