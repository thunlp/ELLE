...
class Trainer(object):
    ...
    def __init__(self, args, task, model, models_distil_from, criterion, dummy_batch=None, oom_batch=None):
        self.args = args
        self.task = task
        ...
        self._model = model
        self._models_distil_from = models_distil_from
        self.cuda = torch.cuda.is_available() and not args.cpu
        if args.fp16:
            self._criterion = self._criterion.half()
            self._model = self._model.half()
            if len(self._models_distil_from) > 0:
                for k in self._models_distil_from.keys():
                    self._models_distil_from[k] = self._models_distil_from[k].half()
        if self.cuda:
            self._criterion = self._criterion.cuda()
            self._model = self._model.cuda()
            if len(self._models_distil_from) > 0:
                for k in self._models_distil_from.keys():
                    self._models_distil_from[k] = self._models_distil_from[k].cuda()
        ...
        self._wrapped_models_distil_from = None
        ...
        self.trained_domains = args.trained_domains

        self.domain_sample_size = {}
        for domain in args.domains:
            self.domain_sample_size[domain] = 0
    ...
    @property
    def models_distil_from(self):
        if self._models_distil_from == None:
            return None
        if self._wrapped_models_distil_from is None:
            self._wrapped_models_distil_from = {}
            if self.args.distributed_world_size > 1 and not self.args.use_bmuf:
                for k in self._models_distil_from.keys():
                    self._wrapped_models_distil_from[k] = models.DistributedFairseqModel(
                        self.args, self._models_distil_from[k]
                    )
            else:
                self._wrapped_models_distil_from = self._models_distil_from
        return self._wrapped_models_distil_from
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            self.trained_domains = state.get("trained_domains", [])
    ...
    def train_step(self, samples, replay=False, dummy_batch=False, raise_oom=False):
        ...
        domain_list = []
        for i, (domain, sample) in enumerate(samples):
            sample = self._prepare_sample(sample)
            if sample is None:
                # when sample is None, run forward/backward on a dummy batch
                # and ignore the resulting gradients
                sample = self._prepare_sample(self._dummy_batch[1])
                domain = self._dummy_batch[0]
                ignore_grad = True
            else:
                ignore_grad = False
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size, logging_output = self.task.train_step(
                        sample, domain, self.model, self.models_distil_from.get(domain, None) if domain else None, self.get_num_updates(), self.criterion, self.optimizer, ignore_grad, replay=replay
                    )
        ...
        if self.fast_stat_sync:
            ...
            for j in range(len(sample_sizes)):
                for i in range(len(sample_sizes[j])):
                    try:
                        self.domain_sample_size[domain_list[i]] += sample_sizes[j][i]
                    except:
                        print(i,j)
                        print(len(samples), len(sample_sizes[j]), len(domain_list))
                        print(len(sample_sizes), self.args.distributed_world_size)
                        raise IndexError
    ...
    def valid_step(self, domain, sample, raise_oom=False):
        ...
        with torch.no_grad():
            ...
            try:
                _loss, sample_size, logging_output = self.task.valid_step(
                    sample, domain, self.model, model_distil_from=None, num_updates=self.get_num_updates(), criterion=self.criterion
                )
            except RuntimeError as e:
                if "out of memory" in str(e):
                    self._log_oom(e)
                    if not raise_oom:
                        ...
                        if self.cuda:
                            torch.cuda.empty_cache()
                        return self.valid_step(domain, sample, raise_oom=True)
                raise e
        ...
        return logging_output, sample_size
    
    def valid_step_with_grad(self, domain, sample, raise_oom=False):
        """Do forward pass in evaluation mode."""
        self.model.eval()
        self.criterion.eval()
        ...
        if sample == None: return Noneb 

        try:
            _loss, sample_size, logging_output= self.task.valid_step_with_grad(
                sample, domain, self.model, model_distil_from=None, num_updates=self.get_num_updates(), criterion=self.criterion, optimizer=self.optimizer
            )
        except RuntimeError as e:
            if "out of memory" in str(e):
                self._log_oom(e)
