...
def save_checkpoint(args, trainer, epoch_itr, val_loss):
    ...
    if len(checkpoints) > 0:
        ...
        shutil.copyfile(checkpoints[0], os.path.join(args.save_dir,"checkpoint_{}_{}.pt".format(args.cur, args.cur_domain)))
        write_timer.stop()
        print(
...
def load_checkpoint(args, trainer, domain, **passthrough_args):
    """
    ...
    *passthrough_args* will be passed through to
    ...
    if extra_state is not None and not args.reset_dataloader:
        ...
        epoch_itr = trainer.get_train_iterator(
            epoch=itr_state["epoch"], load_dataset=True, domain=domain, **passthrough_args
        )
        epoch_itr.load_state_dict(itr_state)
    else:
        epoch_itr = trainer.get_train_iterator(
            epoch=0, load_dataset=True, domain=domain, **passthrough_args
        )
...
def save_state(
    ...
    optim_history=None,
    extra_state=None,
    trained_domains=None, 
):
    from fairseq import utils
    assert len(trained_domains) != 0
    ...
    try:
        ...
        with fb_pathmgr.open(filename, "wb") as f:
            torch_persistent_save(state_dict, f, _use_new_zipfile_serialization=False)
    except (ModuleNotFoundError, ImportError):
        # if path manager not found, continue with local file.
        torch_persistent_save(state_dict, filename, _use_new_zipfile_serialization=False)
